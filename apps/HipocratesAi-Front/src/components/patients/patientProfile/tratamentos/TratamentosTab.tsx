...
import type { Treatment } from '@hipo/contracts';
import { usePatientTreatments } from '../../../../hooks/usePatientClinicalData';
...
export default function TratamentosTab({ patientId }: TratamentosTabProps) {
  ...
  const { data, isLoading, error } = usePatientTreatments(patientId);

  const patientTreatments = data ?? [];
  ...
  if (isLoading) {
    return <div className="text-center py-12 text-gray-500 text-sm">Carregando tratamentos...</div>;
  }

  if (error) {
    return (
      <div className="text-center py-12 text-red-600 text-sm">
        {error instanceof Error ? error.message : 'Erro ao carregar tratamentos.'}
      </div>
    );
  }
