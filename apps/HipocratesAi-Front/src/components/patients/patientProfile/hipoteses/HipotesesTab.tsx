...
import type { Hypothesis } from '@hipo/contracts';
import { usePatientHypotheses } from '../../../../hooks/usePatientClinicalData';
...
export default function HipotesesTab({ patientId }: HipotesesTabProps) {
  ...
  const { data, isLoading, error } = usePatientHypotheses(patientId);

  const patientHypotheses = data ?? [];
  ...
  if (isLoading) {
    return <div className="text-center py-12 text-gray-500 text-sm">Carregando hipóteses...</div>;
  }

  if (error) {
    return (
      <div className="text-center py-12 text-red-600 text-sm">
        {error instanceof Error ? error.message : 'Erro ao carregar hipóteses.'}
      </div>
    );
  }
