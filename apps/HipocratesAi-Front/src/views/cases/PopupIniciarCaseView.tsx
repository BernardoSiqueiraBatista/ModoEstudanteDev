...
export default function IniciarCase({ onClose }: IniciarCaseProps) {
  ...
  const handleStart = () => {
    ...
    console.log(`Iniciando tentativa no modo: ${selectedMode.toUpperCase()}`);
    
    onClose();
    if (selectedMode === 'osce') {
      navigate('/osce'); 
    } else if (selectedMode === 'hm') {
      navigate('/hm'); 
    }
  };
