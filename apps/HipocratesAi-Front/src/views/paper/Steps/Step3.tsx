...
export default function Step3({ onNext, onBack, onClose }: { onNext: () => void; onBack: () => void; onClose: () => void }) {
  const [instructions, setInstructions] = useState(() => localStorage.getItem('setup-plano-instructions') || '');
  const [horasDia, setHorasDia] = useState(() => Number(localStorage.getItem('setup-plano-horas-dia') || '2'));
  const [fixedEvents, setFixedEvents] = useState<FixedEvent[]>(() => {
    try { return JSON.parse(localStorage.getItem('setup-plano-compromissos') || '[]'); }
    catch { return []; }
  });
  const [showModal, setShowModal] = useState(false);

  useEffect(() => { localStorage.setItem('setup-plano-instructions', instructions); }, [instructions]);
  useEffect(() => { localStorage.setItem('setup-plano-horas-dia', String(horasDia)); }, [horasDia]);
  useEffect(() => { localStorage.setItem('setup-plano-compromissos', JSON.stringify(fixedEvents)); }, [fixedEvents]);
        ...
        <section className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-3">
            ...
            <textarea
              ...
              className="h-32 w-full resize-none rounded-2xl border border-outline-variant/20 bg-white/60 p-5 text-on-surface backdrop-blur-md transition-all duration-300 placeholder:text-outline focus:border-primary focus:outline-none focus:ring-4 focus:ring-primary/10"
            />
          </div>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <label className="text-[15px] font-semibold text-on-surface">Horários Bloqueados</label>
              </div>
              <button
                type="button"
                onClick={() => setShowModal(true)}
                className="flex items-center gap-1.5 rounded-full border border-primary/20 bg-primary/5 px-4 py-1.5 text-xs font-bold text-primary hover:bg-primary/10 transition-colors"
              >
            ...
            <p className="text-xs text-on-surface-variant leading-relaxed">
              Períodos em que você <strong>não pode estudar</strong>. A IA os respeitará ao gerar o cronograma.
            </p>

            {fixedEvents.length === 0 ? (
              <div className="flex items-center gap-3 rounded-2xl border border-dashed border-outline-variant/30 bg-white/40 p-4">
                <span className="material-symbols-outlined text-[20px] text-on-surface-variant/40">block</span>
                <p className="text-sm text-on-surface-variant/60">Nenhum horário bloqueado adicionado.</p>
              </div>
            ) : (
