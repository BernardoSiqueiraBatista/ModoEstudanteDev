...
function fixedEventsToPayload(events: FixedEvent[]) {
  return events.flatMap(ev =>
    ev.days.map(day => ({
      dia: DAY_MAP[day] ?? day.toLowerCase().slice(0, 3),
      inicio: ev.startTime,
      fim: ev.endTime,
      label: ev.name,
    }))
...
export default function Step4({ onBack, onClose, onFinish }: { onBack: () => void; onClose: () => void; onFinish: () => void }) {
  ...
  const savedFixedEvents: FixedEvent[] = (() => {
  ...
  async function handleFinish() {
    ...
    try {
      ...
      const compromissos_fixos = fixedEventsToPayload(savedFixedEvents);
      ...
      const res = await fetch(`${API}/student/v1/study-plans`, {
        ...
        body: JSON.stringify({
          ...
          ...(compromissos_fixos.length > 0 ? { compromissos_fixos } : {}),
      ...
      ['setup-plano-areas', 'setup-plano-ciclo', 'setup-plano-instructions', 'setup-plano-horas-dia', 'setup-plano-categoria', 'setup-plano-upload-id', 'setup-plano-compromissos'].forEach(k => localStorage.removeItem(k));
      onFinish();
    } catch (e) {
    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto px-6 md:px-10 py-6 relative z-10">
        <section className="space-y-5">
          ...
          {/* Compromissos fixos (se houver) */}
          {savedFixedEvents.length > 0 && (
            <div className="rounded-[1.5rem] border border-white/40 bg-white/60 backdrop-blur-md p-4 shadow-[0_10px_30px_rgba(0,0,0,0.03)]">
              <span className="block mb-3 text-[10px] uppercase tracking-[0.18em] font-bold text-secondary">Horários Bloqueados</span>
              <div className="flex flex-wrap gap-2">
                {savedFixedEvents.map((ev, i) => (
                  <span key={i} className="inline-flex items-center gap-1.5 rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">
                    <span className="material-symbols-outlined text-[12px]">event_busy</span>
                    {ev.name} · {ev.days.join(', ')} · {ev.startTime}–{ev.endTime}
                  </span>
                ))}
              </div>
            </div>
